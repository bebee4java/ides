...
class HiveDataSource extends DataReader with DataWriter {
  ...
  override def load(reader: Reader, config: DataSourceConfig): DataTable = {
    ...
    val dataFrame = sparkDataReader.options(config.config).format(format).table(config.path)
...
//      fieldDelim, escapeDelim, collectionDelim, mapkeyDelim, lineDelim
    ...
    sparkDataWriter.format(format).options(options).mode(saveMode).saveAsTable(config.path)
  }
